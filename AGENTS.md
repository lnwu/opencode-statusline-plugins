...
| `scripts/record-demo.ts` | Dev-only demo recorder: a real TUI in a fixed tmux size wrapped in `terminal-svg rec`, driven with `send-keys`, rendered to `assets/demo.{en,zh-CN}.svg` (plus `.cast` masters). Runbook: the `record-demo` skill. |
