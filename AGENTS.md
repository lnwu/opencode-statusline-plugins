...
| `src/usage-cache.ts` | `createUsageCache`: server-side TTL + single-flight wrapper for a plugin's quota fetch, so several TUI clients share one upstream request; inlined into each package's `dist/index.js`. |
...
| `test/language.test.ts`, `test/theme.test.ts`, `test/usage-cache.test.ts` | Unit tests; run in the `checks` CI job. |
...
| `src/usage.ts` | Pure mapping from the `zen/go/v1/usage` response to the displayed usage (rolling 5h, weekly, and monthly windows; rejects windows with malformed fields). |
...
| `test/usage.test.ts` | Unit tests for the `src/usage.ts` mapping; no credential needed. |
...
| `test/usage.test.ts` | Unit tests for the `src/usage.ts` mapping (ratio and absolute fallbacks, per-window fallback, string numbers, clamping); no credential needed. |
