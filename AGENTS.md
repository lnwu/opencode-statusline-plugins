...
| `src/record-demo.ts` | `recordDemo`: local-mode README demo recorder (real TUI in tmux, wrapped in `terminal-svg rec`, rendered to an animated SVG); takes a list of takes (prompt, model, env, output name) so it carries no locale/label knowledge, plus a gitignored per-package `record-demo.config.json` for per-developer defaults. Hides the cursor by default, pins each take's model in opencode's `model.json` (home-screen model selection) while recording, hands an optional TUI theme (`tuiTheme`) to the recorded process as inline CLI settings, and repairs terminal-svg's chunk-boundary glyph corruption (russmckendrick/terminal-svg#4). Runbook: the `record-demo` skill. |
