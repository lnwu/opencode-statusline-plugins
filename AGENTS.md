...
| `assets/` | README screenshots. Repo-only (kept out of the tarball by `files: ["dist"]`); referenced by absolute raw URLs. |
...
- Package screenshots live in `packages/<pkg>/assets/` and are referenced by
  absolute raw URLs — npm renders the README but does not resolve relative
  image paths. They come from a real tmux capture (`capture-pane -peN`) of the
  TUI, rendered with freeze + `@resvg/resvg-js` (Cascadia Next SC covers the
  CJK labels). `files: ["dist"]` keeps them out of the tarball.
